...
func runGetObjectBenchmark(b *testing.B, obj ObjectLayer, objSize int) {
	...
	for i := 0; i < b.N; i++ {
		...
		err = obj.GetObject(bucket, "object"+strconv.Itoa(i%10), 0, int64(objSize), buffer, nil)
		if err != nil {
			b.Error(err)
		}
	}
...
func runGetObjectBenchmarkParallel(b *testing.B, obj ObjectLayer, objSize int) {
	...
	b.RunParallel(func(pb *testing.PB) {
		i := 0
		for pb.Next() {
			err = obj.GetObject(bucket, "object"+strconv.Itoa(i), 0, int64(objSize), ioutil.Discard, nil)
			if err != nil {
				b.Error(err)
			}
			i++
